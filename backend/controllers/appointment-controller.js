...
export const getAppointments = async (req, res) => {
    try {
        ...
        const item = await Appointment.find(filter).sort({ createdAt: -1}).skip(skip).limit(limit)
            ...
            return res.json({
                success: true,
                appointments: item,
                meta: {page, limit, total, count: item.length}
            })
    }
...
export const confirmPayment = async (req, res) => {
  try {
    ...
    console.error("ConnfirmPayment error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
