import Servise  from "../models/Service.js";
import { uploadToCloudinary, deleteFromCloudinary } from "../utils/cloudinary.js";

// Helpers Function
//so this functionn converts array like input into a clean array
...
export async function getServices(req, res) {
  try {
    const list = await Servise.find().sort({ createdAt: -1}).lean();
...
export async function getServiceById(req, res) {
  try {
    const {id} = req.params;
    const service = await Servise.findById(id).lean();
...
export async function updateService(req, res) {
  try {
    const { id } = req.params;
    const existing = await Servise.findById(id);
    ...
    const updated = await Servise.findByIdAndUpdate(id, updateData, { 
      new: true,
      runValidators: true
    });
    return res.status(200).json({
